...
const register = async(req,res) => {
    ...
    if (userexists) return res.status(400).json({msg:'User with this phone exists!'});

    const passhash =  await bcrypt.hash(password,10);
    const user = await User.create({email,phone,password:passhash});
    const token = jwt.sign({id : user._id},process.env.jwt_token,{expiresIn:'7d'});
    res.status(201).json({user:{id:user._id,email,phone,name},token});
} 
